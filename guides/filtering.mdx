...
```bash Authors by name
curl "https://api.openalex.org/authors?filter=display_name.search:einstein"
```
...
| Entity | Filters |
|--------|---------|
| [Works](/api-reference/works/list-works) | `publication_year`, `type`, `open_access.is_oa`, `author.id`, `institutions.id`, `cited_by_count`, `doi`, `has_abstract`, and ~150 more |
| [Authors](/api-reference/authors/list-authors) | `has_orcid`, `last_known_institutions.id`, `works_count`, `cited_by_count`, `orcid` |
| [Sources](/api-reference/sources/list-sources) | `is_oa`, `is_in_doaj`, `type`, `country_code`, `host_organization`, `issn` |
| [Institutions](/api-reference/institutions/list-institutions) | `country_code`, `type`, `is_global_south`, `continent`, `ror`, `has_ror` |
| [Topics](/api-reference/topics/list-topics) | `domain.id`, `field.id`, `subfield.id`, `works_count` |
| [Keywords](/api-reference/keywords/list-keywords) | `works_count`, `cited_by_count` |
| [Publishers](/api-reference/publishers/list-publishers) | `country_codes`, `hierarchy_level`, `ror`, `parent_publisher` |
| [Funders](/api-reference/funders/list-funders) | `country_code`, `is_global_south`, `continent`, `ror`, `awards_count` |

<Info>
**Looking for text search?** Filters match exact values. For full-text search in titles, abstracts, and other fields, see [Searching](/guides/searching). For AI-powered semantic search that finds conceptually related works, see [Similar works](/guides/similar-works).
</Info>
