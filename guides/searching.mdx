...
<Note>
Semantic search is limited to **1 request per second**.
</Note>

Semantic search returns a maximum of **50 results** per query. Because every work relates to every query to some degree, the engine evaluates a pool of candidates and returns only the top 50 by similarity.
