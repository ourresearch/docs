...
```
/works?search.semantic=<query>       - Semantic search (100 credits)
content.openalex.org/works/{id}.pdf - Download PDFs (100 credits)
...
```bash
/works?filter=doi:10.1234/a|10.1234/b|10.1234/c&per_page=200
```
...
| List (`/works?filter=...`) | 1 |
| Search (`?search=` or `.search` filters) | 10 |
| Content download (PDF) | 100 |
| Semantic search (`?search.semantic=`) | 100 |
...
| Default page size | Use `per_page=200` |
