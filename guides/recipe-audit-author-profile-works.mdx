...
OpenAlex's author entity resolution is good but imperfect. A profile (`A1234...`) can be missing real works (the byline used a name variant the resolver didn't link), and can also include works that belong to someone else (a different person with a similar byline got attached). This recipe finds both kinds of candidate so you can correct the profile. <Tooltip headline="Under 1¢ per profile" tip="~5–10 List+Filter calls per profile at $0.10 per 1,000"><Icon icon="coins" size={16} color="#DAA520" /></Tooltip>
...
def candidate_tokenizations(name):
    """Yield possible token orderings for a byline. Bylines often look like
    'Last, First' but sometimes are 'First, Last' with a stray comma — try both."""
    s = _normalize(name)
    if "," in s:
        last, _, rest = s.partition(",")
        yield [t for t in f"{rest.strip()} {last.strip()}".split() if t]   # flipped
        yield [t for t in s.replace(",", " ").split() if t]                # as-written
    else:
        yield [t for t in s.split() if t]
...
def byline_matches_seed(raw_name, seed_tokens):
    for cand in candidate_tokenizations(raw_name):
        if not cand or cand[-1] != seed_tokens[-1]:
            continue
        if len(seed_tokens) == 1 or given_match(seed_tokens[0], cand[0]):
            return True
    return False
...
<Warning>
**False-positive classes worth knowing before you remove anything:** married/maiden-name changes, transliteration variants of non-Latin names (e.g. Chinese pinyin vs Wade-Giles), single-name authors, and authors who publish under a pseudonym. A surname mismatch is a *signal*, not a verdict — always corroborate via ORCID, co-authors, or institution before removing.
</Warning>
...
def name_tokens(name):
    """Tokenize a name with the conventional 'Last, First' comma-flip."""
    s = _normalize(name)
    if "," in s:
        last, _, rest = s.partition(",")
...
def candidate_tokenizations(name):
    """Yield both 'Last, First' (flipped) and 'as-written' orderings for bylines with commas."""
    s = _normalize(name)
    if "," in s:
        last, _, rest = s.partition(",")
        yield [t for t in f"{rest.strip()} {last.strip()}".split() if t]
        yield [t for t in s.replace(",", " ").split() if t]
    else:
        yield [t for t in s.split() if t]
...
`additions` are candidates to attach via [the curation API](/guides/curation-authors); `removals` are candidates to detach. Both lists are *candidates* — score them with the signals above and route low-confidence cases through human review.
