...
| **Search** — Full-text and semantic search | `/works?search=climate+change` | $1 / 1,000 calls |
| **Content Download** — Cached PDF via content API | `content.openalex.org/works/{id}.pdf` | $10 / 1,000 downloads |
| **Text/Aboutness** *(deprecated)* | `/text/topics` | $0.10 / call |
...
| Aboutness (`/text`) | $0.10 / call | ~10 requests | Deprecated |
...
<Note>
The `/rate-limit` response uses internal units where $1 = 10,000 units.
</Note>
