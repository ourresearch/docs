...
The complete OpenAlex database as gzip-compressed JSON Lines files, updated monthly. Includes works, authors, sources, institutions, topics, publishers, funders, and more.

**Best for:** Full database replication, data warehousing, comprehensive analysis

**Size:** ~330 GB compressed, ~1.6 TB decompressed
...
Download PDFs and TEI XML for about 60 million works. Requires an API key — content downloads cost 100 credits per file.
...
<CardGroup cols={2}>
  <Card title="Data warehouse" icon="database" href="/download/data-warehouse">
    Load JSON directly into BigQuery, Snowflake, or similar. Query with native JSON functions.
  </Card>
  <Card title="Relational database" icon="table" href="/download/relational-database">
    Flatten to normalized tables in PostgreSQL or MySQL. Simpler queries, more setup.
  </Card>
</CardGroup>
