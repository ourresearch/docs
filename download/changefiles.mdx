...
<Warning>
**Daily changefiles require a paid plan.** The free public snapshot is refreshed **quarterly**. **Monthly snapshots** and the **daily changefiles** documented on this page (the `/changefiles` API) are only available to subscribers on a [paid plan](https://openalex.org/pricing). Contact [sales@openalex.org](mailto:sales@openalex.org).
</Warning>
